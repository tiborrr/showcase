    ...
    <div>
      <ul class="list-group">
        <li v-for="post in posts" :key="post.id" class="list-group-item">
          <h3>{{ post.title }}</h3>
          <p>{{ post.content }}</p>
          <button @click="togglePostEditing(post.id)" class="btn btn-secondary">
            {{ isEditingPost(post.id) ? 'Cancel' : 'Edit' }}
          </button>
          ...
          <div class="mt-3">
            <button @click="togglePostCommenting(post.id)" class="btn btn-primary">
              {{ isCommentingOnPost(post.id) ? 'Cancel' : 'Comment' }}
            </button>
            <CommentCreate v-if="isCommentingOnPost(post.id)" :postId="post.id" @refresh-posts="refreshPosts" />
          </div>
        </li>
      </ul>
    </div>
